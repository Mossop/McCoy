...
const XML_EXTENDER =
        "\\u00B7\\u02D0\\u02D1\\u0387\\u0640\\u0E46\\u0EC6\\u3005" + 
        "\\u3031-\\u3035\\u309D-\\u309E\\u30FC-\\u30FE";
const XML_NCNAMECHAR = XML_LETTER + XML_DIGIT + "\\.\\-_" + XML_COMBINING + XML_EXTENDER;
...
const Attr = Ci.nsIDOMAttr;
const Text = Ci.nsIDOMText;
...
var EXPORTED_SYMBOLS = [ "RDFLiteral", "RDFIntLiteral", "RDFDateLiteral",
                         "RDFBlankNode", "RDFResource", "RDFDataSourceFactory" ];
...
function renameNode(domnode, namespaceURI, qname) {
  if (domnode instanceof Element) {
    ...
    for (var i = domnode.attributes.length - 1; i >= 0; i--) {
  ...
  else if (domnode instanceof Attr) {
    ...
    throw "cannot rename node of this type";
  }
}
 ...
 */
function RDFAssertion(subject, predicate, object) {
  if (!(subject instanceof RDFSubject))
    throw "subject must be an RDFSubject";

  if (typeof(predicate) != "string")
    throw "predicate must be a string URI";

  if (!(object instanceof RDFLiteral) && !(object instanceof RDFSubject))
    throw "object must be a concrete RDFNode";

  if (object instanceof RDFSubject && object._ds != subject._ds)
    throw "object must be from the same datasource as subject";
...
RDFAssertion.prototype = {
   ...
   * attached to.
   */
  _getSubjectElement: function() {
    if (this._DOMnode instanceof Attr)
 ...
 */
function RDFIntLiteral(value) {
  RDFLiteral.call(this, parseInt(value));
}

RDFIntLiteral.prototype = new RDFLiteral();

/**
 * This stores the value of the literal in the given DOM node
 */
RDFIntLiteral.prototype._applyToDOMNode = function(ds, domnode) {
  if (!(domnode instanceof Element))
    throw "cannot use this node for a literal";
...
RDFIntLiteral.prototype.equals = function(rdfnode) {
 ...
 */
function RDFDateLiteral(value) {
  if (!(value instanceof Date))
    throw "RDFDateLiteral must be constructed with a Date object";

  RDFLiteral.call(this, value);
}

RDFDateLiteral.prototype = new RDFLiteral();

/**
 * This stores the value of the literal in the given DOM node
 */
RDFDateLiteral.prototype._applyToDOMNode = function(ds, domnode) {
  if (!(domnode instanceof Element))
    throw "cannot use this node for a literal";
...
RDFDateLiteral.prototype.equals = function(rdfnode) {
 ...
 */
RDFSubject.prototype._removeElement = function(element) {
  ...
  for (var predicate in this._assertions) {
    for (var i = 0; i < this._assertions[predicate].length; i++) {
      var assertion = this._assertions[predicate][i];
 ...
 */
RDFSubject.prototype._createDOMNodeForAssertion = function(assertion) {
  if (RDF_LISTITEM.test(assertion.getPredicate())) {
    // Find all the containers
    var elements = this._elements.filter(function(element) {
    ...
    if (elements.length > 0) {
      ...
      for (i = 0; i < elements.length; i++) {
        if (elements[i].listCounter == item) {
          assertion._DOMnode = this._ds._addElement(elements[i], RDF_R("li"));
          assertion._applyToDOMNode();
          elements[i].listCounter++;
          return;
        }
      }
    ...
    for (var i = 0; i < this._elements.length; i++) {
      if (this._elements[i].namespaceURI == NS_RDF &&
          this._elements[i].localName == "Description") {
        try {
          var prefix = this._ds._resolvePrefix(this._elements[i].parentNode, assertion.getObject().getURI());
          this._elements[i] = renameNode(this._elements[i], prefix.namespaceURI, prefix.qname);
          assertion._DOMnode = this._elements[i];
 ...
 */
RDFSubject.prototype._removeDOMNodeForAssertion = function(assertion) {
  if (assertion._DOMnode instanceof Attr) {
    ...
    if (object instanceof RDFSubject && assertion._DOMnode.firstChild) {
      ...
      for (var i = 0; i < object._elements.length; i++) {
        if (object._elements[i].parentNode == assertion._DOMnode) {
          object._removeElement(object._elements[i]);
          break;
        }
      }
    }
  ...
  for (var p in this._assertions) {
    for (var j = 0; j < this._assertions[p].length; j++) {
      if (this._assertions[p][j]._getSubjectElement() == parent)
        return;
    }
  }
 ...
 */
RDFSubject.prototype._parseElement = function(element) {
  this._elements.push(element);
  ...
  if (element.namespaceURI != NS_RDF ||
    ...
    if (element.namespaceURI == NS_RDF && element.localName == "li")
      throw "rdf:li is not a valid type for a subject node";
  ...
  for (var i = 0; i < element.attributes.length; i++) {
    ...
    if ((attr.namespaceURI == NS_RDF || !attr.namespaceURI) &&
        (["nodeID", "about", "resource", "ID", "parseType"].indexOf(attr.localName) >= 0))
      continue;
    var object = null;
    if (attr.namespaceURI == NS_RDF) {
      if (attr.localName == "type")
        ...
        throw "rdf:li is not allowed as a property attribute"
      else if (attr.localName == "aboutEach")
        throw "rdf:aboutEach is deprecated"
      else if (attr.localName == "aboutEachPrefix")
        throw "rdf:aboutEachPrefix is deprecated"
      else if (attr.localName == "aboutEach")
        throw "rdf:aboutEach is deprecated"
      else if (attr.localName == "bagID")
        throw "rdf:bagID is deprecated"
    }
    if (!object)
  ...
  element.listCounter = 1;
  while (child) {
    if (child instanceof Text && /\S/.test(child.nodeValue)) {
      ERROR("Text " + child.nodeValue + " is not allowed in a subject node");
      throw "subject nodes cannot contain text content";
    }
    else if (child instanceof Element) {
      object = null;
      var predicate = child.namespaceURI + child.localName;
      if (child.namespaceURI == NS_RDF) {
        if (RDF_PROPERTY_INVALID_TYPES.indexOf(child.localName) >= 0 &&
            !child.localName.match(/^_\d+$/))
          throw child.nodeName + " is an invalid property";
      ...
      for (var i = 0; i < child.attributes.length; i++) {
        ...
        if (child.attributes[i].namespaceURI == NS_XML)
          continue;
        // These are reserved by XML for future use
        if (child.attributes[i].localName.substring(0, 3).toLowerCase() == "xml")
          continue;
        // We can handle these RDF attributes
        if ((!child.attributes[i].namespaceURI || child.attributes[i].namespaceURI == NS_RDF) &&
            ["resource", "nodeID"].indexOf(child.attributes[i].localName) >= 0)
          continue;
        // This is a special attribute we handle for compatibility with Mozilla RDF
        if (child.attributes[i].namespaceURI == NS_NC &&
            child.attributes[i].localName == "parseType")
          continue;
        ERROR("Attribute " + child.attributes[i].nodeName + " is not supported");
        throw "Attribute " + child.attributes[i].nodeName + " is not supported";
      }
      ...
      if ((resource && (nodeID || parseType)) || 
        ...
        throw "invalid rdf assertion";
      }

      if (resource !== undefined) {
        var base = ioService.newURI(element.baseURI, "UTF-8", null);
        object = this._ds.getResource(base.resolve(resource));
      }
      else if (nodeID !== undefined) {
        if (!nodeID.match(XML_NCNAME))
          throw "rdf:nodeID must be a valid XML name";
        ...
        while (subchild) {
          if (subchild instanceof Text && /\S/.test(subchild.nodeValue)) {
            hasText = true;
          }
          else if (subchild instanceof Element) {
            if (childElement) {
              ERROR("Multiple object elements found in " + child.nodeName);
              throw "assertion cannot contain multiple objects";
            }
        ...
        if (childElement) {
          if (parseType) {
            ...
            throw "parseType is not valid in this context";
          }
 ...
 */
RDFSubject.prototype._addAssertion = function(assertion) {
 ...
 */
RDFSubject.prototype._removeAssertion = function(assertion) {
  var predicate = assertion.getPredicate();
  if (predicate in this._assertions) {
 ...
 */
RDFSubject.prototype._getChildAssertions = function() {
  var assertions = [];
  for (var i in this._assertions) {
    if (RDF_LISTITEM.test(i))
      this._assertions[i].forEach(function (a) { assertions.push(a) });
  }
 ...
 */
RDFSubject.prototype.equals = function(rdfnode) {
 ...
 */
RDFSubject.prototype.assert = function(predicate, object) {
  if (predicate == RDF_R("type") && !(object instanceof RDFResource))
    throw "rdf:type must be an RDFResource";
 ...
 * assertion exists.
 */
RDFSubject.prototype.unassert = function(predicate, object) {
  if (!(predicate in this._assertions))
    return;
 ...
 * subject.
 */
RDFSubject.prototype.getPredicates = function() {
  return [i for (i in this._assertions)];
};

/**
 * Returns all objects in assertions with this subject and the given predicate.
 */
RDFSubject.prototype.getObjects = function(predicate) {
  if (predicate in this._assertions)
    return [i.getObject() for each (i in this._assertions[predicate])];
 ...
 */
RDFSubject.prototype.getChildren = function() {
  var assertions = this._getChildAssertions();
  return [i.getObject() for each (i in assertions)];
};
 ...
 * children.
 */
RDFSubject.prototype.removeChildAt = function(pos) {
  if (pos < 0)
    throw "no such child";
  var assertions = this._getChildAssertions();
  ...
  for (i = pos + 1; i< assertions.length; i++) {
 ...
 */
RDFSubject.prototype.removeChild = function(object) {
  var assertions = this._getChildAssertions();
 ...
 */
RDFSubject.prototype.addChild = function(object) {
  var max = 0;
  for (var i in this._assertions) {
    if (RDF_LISTITEM.test(i))
 ...
 */
RDFSubject.prototype.reorderChildren = function() {
  var assertions = this._getChildAssertions();
 ...
 */
RDFSubject.prototype.getType = function() {
 ...
 */
RDFSubject.prototype.hasProperty = function(predicate) {
 ...
 */
RDFSubject.prototype.getProperty = function(predicate) {
  if (predicate in this._assertions)
 ...
 * values.
 */
RDFSubject.prototype.setProperty = function(predicate, object) {
 ...
 */
RDFSubject.prototype.clearProperty = function(predicate) {
  if (!(predicate in this._assertions))
    return;
 ...
 */
function RDFResource(ds, uri) {
  if (!(ds instanceof RDFDataSource))
    throw "datasource must be an RDFDataSource";

  if (!uri)
    throw "An RDFResource requires a non-null uri";

  RDFSubject.call(this, ds);
  this._uri = uri;
}

RDFResource.prototype = new RDFSubject();

// This is the uri that the resource represents.
RDFResource.prototype._uri = null;

/**
 * Sets attributes on the DOM element to mark it as representing this resource
 */
RDFResource.prototype._applyToElement = function(element) {
  if (USE_RDFNS_ATTR) {
 ...
 */
RDFResource.prototype._addReferenceToElement = function(element) {
  if (USE_RDFNS_ATTR) {
 ...
 */
RDFResource.prototype._removeReferenceFromElement = function(element) {
 ...
 */
function RDFBlankNode(ds, nodeID) {
  if (!(ds instanceof RDFDataSource))
    throw "datasource must be an RDFDataSource";
...
RDFBlankNode.prototype = new RDFSubject();

// The nodeID of this node. May be null if there is no ID.
RDFBlankNode.prototype._nodeID = null;

/**
 * Sets attributes on the DOM element to mark it as representing this node
 */
RDFBlankNode.prototype._applyToElement = function(element) {
  if (!this._nodeID)
    return;
  if (USE_RDFNS_ATTR) {
 ...
 * subject. The URI controls what tagname to use.
 */
RDFBlankNode.prototype._createNewElement = function(uri) {
  ...
  RDFSubject.prototype._createNewElement.call(this, uri);
}

/**
 * Adds a reference to this node to the given property Element.
 */
RDFBlankNode.prototype._addReferenceToElement = function(element) {
  ...
  if (this._nodeID) {
    if (USE_RDFNS_ATTR) {
      prefix = this._ds._resolvePrefix(element, RDF_R("nodeID"));
      element.setAttributeNS(prefix.namespaceURI, prefix.qname, this._nodeID);
    }
    else {
      element.setAttribute("nodeID", this._nodeID);
 ...
 */
RDFBlankNode.prototype._removeReferenceFromElement = function(element) {
...
RDFBlankNode.prototype.getNodeID = function() {
  return this._nodeID;
};
 ...
 */
function RDFDataSource(document) {
  this._resources = {};
  this._allBlankNodes = [];
  this._blankNodes = {};
  this._prefixes = {
...
RDFDataSource.prototype = {
   ...
   */
  _createNodeID: function(blanknode) {
    var i = 1;
   ...
   */
  _getSubjectForElement: function(element) {
    if (element.namespaceURI == NS_RDF &&
        RDF_NODE_INVALID_TYPES.indexOf(element.localName) >= 0)
      throw element.nodeName + " is not a valid class for a subject node";
    ...
    if (id !== undefined) {
      if (!id.match(XML_NCNAME))
        throw "rdf:ID must be a valid XML name";
      var base = ioService.newURI(element.baseURI, "UTF-8", null);
  ...
  _parseDocument: function() {
    if (!this._document.documentElement) {
      ...
      throw "document contains no root element";
    }
    ...
    if (this._document.documentElement.namespaceURI != NS_RDF ||
      ...
      throw "document does not appear to be RDF";
    }

    var domnode = this._document.documentElement.firstChild;
    while (domnode) {
      if (domnode instanceof Text && /\S/.test(domnode.nodeValue)) {
        ERROR("RDF does not allow for text in the root of the document");
        throw "invalid markup in document";
      }
   ...
   */
  _resolvePrefix: function(domnode, uri, badPrefixes) {
    if (!badPrefixes)
      badPrefixes = [];

    // No known prefix, try to create one from the lookup list
    if (!domnode || domnode instanceof Document) {
      for (var i in this._prefixes) {
        if (badPrefixes.indexOf(i) >= 0)
          continue;
      ...
      if (test) {
        ...
        local = test[0];
        var i = 1;
        while (badPrefixes.indexOf("NS" + i) >= 0)
          i++;
    ...
    for (var i = 0; i < domnode.attributes.length; i++) {
      ...
      if (badPrefixes.indexOf(prefix) >= 0)
        continue;
  ...
  _guessIndent: function(element) {
    ...
    while (sibling && sibling instanceof Text) {
  ...
  _addElement: function(parent, uri) {
    ...
    if (parent.lastChild) {
      ...
      while (last && last instanceof Text)
      ...
      var indent = this._guessIndent(parent);
      ...
      var indent = this._guessIndent(parent.parentNode);
   ...
   * document.
   */
  registerPrefix: function(prefix, namespaceURI) {
   ...
   */
  getAllBlankNodes: function() {
    return [i for each (i in this._allBlankNodes)];
  },
  ...
  getAllResources: function() {
    return [i for each (i in this._resources)];
  },
  ...
  getAllSubjects: function() {
    var subjects = [i for each (i in this._resources)];
    for (var i = 0; i < this._allBlankNodes.length; i++)
      subjects.push(this._allBlankNodes[i]);
    return subjects;
  },
  ...
  saveToStream: function(stream) {
    var serializer = Cc["@mozilla.org/xmlextras/xmlserializer;1"].
                     createInstance(Ci.nsIDOMSerializer);
    serializer.serializeToStream(this._document, stream, null);
  },
  ...
  saveToFile: function(file) {
    ...
    var modeFlags = 0x02 | 0x08 | 0x20;    // WRITE, CREATE, TRUNCATE
    fos.init(file, modeFlags, 0664, 0);
...
/**
 * Simple factory to create and load datasources.
 */
RDFDataSourceFactory = {
  ...
  loadFromFile: function(file, uri) {
    if (!uri)
    ...
    var parser = Cc["@mozilla.org/xmlextras/domparser;1"].
                 createInstance(Ci.nsIDOMParser);
    parser.init(null, uri, null);
    var document = parser.parseFromStream(fis, null, file.fileSize, "text/xml");
    fis.close();
